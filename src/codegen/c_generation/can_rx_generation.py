        ...
        for msg in self._db.messages_received_by_node(self._node):
            for signal in msg.signals:
                ...
                if signal.datatype() == CanSignalDatatype.BOOL:
                    update_body = '''\
    {rx_data_var}.{msg_signals_var}.{signal_value_var} = value;'''
                else:
                    update_body = '''\
    {rx_data_var}.{msg_signals_var}.{signal_value_var} = {clamp_func_name}(value);'''
...
class AppCanRxHeaderGenerator(AppCanRxFileGenerator):
    ...
    def _init_structs(self):
        ...
        self._structs.append(CStruct(
            ...
            comment=f'Struct for holding all message data received by {self._node} (i.e. the CAN table).'
        ))
