...
class IoCanTxModule(CModule):
    ...
    def _functions(self) -> List[CFunc]:
        ...
        for cycle_time, freq in MSG_CYCLE_TIME_2_FREQ.items():
            func = CFunc(CFuncsCfg.IO_TX_ENQUEUE_PERIODIC.format(freq=freq), "void", args=[],
            ...
            for msg in self._db.tx_msgs_for_node(self._node):
                if  msg.is_periodic() and msg.cycle_time == cycle_time:
                    func.body.add_line(f"{CFuncsCfg.IO_TX_SEND.format(msg=msg.name, mode=freq)}();")

            public_funcs.append(func)

        # Generate msg sending functions
        for msg in self._db.tx_msgs_for_node(self._node):
            mode = MSG_CYCLE_TIME_2_FREQ[msg.cycle_time] if msg.is_periodic() else "Aperiodic"
            func = CFunc(CFuncsCfg.IO_TX_SEND.format(msg=msg.name, mode=mode), "void", args=[], 
            ...
            func.body.add_line(f"tx_msg.std_id = {CMacrosCfgs.ID.format(msg=msg.name)};")
            func.body.add_line(f"tx_msg.dlc = {CMacrosCfgs.BYTES.format(msg=msg.name)};")
            func.body.add_line()

            # Pack payload
            func.body.add_comment("Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)")
            func.body.add_line("vPortEnterCritical();")
            func.body.add_line(f"{CFuncsCfg.UTILS_PACK.format(msg=msg.name)}({CFuncsCfg.APP_TX_GET_MSG.format(msg=msg.name)}(), tx_msg.data);")
